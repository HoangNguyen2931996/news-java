			...
			<div class="panel panel-default card-view">
				<div class="panel-wrapper collapse in">
					<div class="panel-body">
						<div class="form-wrap">
							<form action="${pageContext.request.contextPath }/admin/link/edit/${objLink.id }" method="post">
								...
								<div class="row">
									<div class="col-md-12">
										<div class="form-group">
											...
											<input name="link" type="text" id="link" class="form-control" placeholder="Tên danh mục" value="${objLink.link }">
										</div>
									</div>
									<!--/span-->
								</div>
									...
									<div class="col-md-6">
										<div class="form-group">
											...
											<c:forEach items="${alResource }" var="objItem">
												<option <c:if test="${objItem.id == objLink.idResource }">selected</c:if> value="${objItem.id }">${objItem.name }</option>
											</c:forEach>
											</select>
										</div>
									</div>	
									<div class="col-md-6">
										<div class="form-group">
											...
											<c:forEach items="${alCat }" var="objItem">
												<option <c:if test="${objItem.id == objLink.idCat }">selected</c:if> value="${objItem.id }">${objItem.name }</option>
											</c:forEach>
											</select>
										</div>
									</div>	
